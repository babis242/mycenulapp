// src/features/referentiel/jumelages/pages/CreerJumelagePage.tsx
import { useEffect, useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { ArrowLeft, Loader2, Search, AlertTriangle } from 'lucide-react';
import { supabase } from '@/lib/supabase';
import { listUEsPourJumelage, createJumelage, type UEOption } from '../api';
...
// Créer un jumelage : choisir un nom, sélectionner au moins 2 UEs
...
export default function CreerJumelagePage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!nom.trim()) {
      setError('Le nom du jumelage est obligatoire.');
      return;
    }
    if (ueIdsSelectionnes.size < 2) {
      setError('Sélectionne au moins 2 UEs à jumeler.');
      return;
    }

    setSaving(true);
    try {
      await createJumelage({
        nom: nom.trim(),
        ue_ids: Array.from(ueIdsSelectionnes),
        enseignant_id: enseignantId || undefined,
      });
      navigate('/referentiel/jumelages');
    } catch (err) {
      setError(
                  ...
                  >
                    ...
                    {u.deja_jumelee && (
                      <span className="flex items-center gap-1 text-[10px] font-bold text-amber-700 bg-amber-100 px-2 py-1 rounded-full shrink-0">
                        <AlertTriangle size={10} /> déjà jumelée
                      </span>
                    )}
                  </label>
                ))}
